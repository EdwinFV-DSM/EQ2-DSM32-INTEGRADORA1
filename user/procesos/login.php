        ...
        if ($conteo0) {
            ...
            if ($conteo0) {
                ...
                if (password_verify($pass, $row['password'])) {
                    ...
                    $querycliente = "SELECT * FROM escuelas WHERE email='$email'";
                    $cliente = mysqli_query($conexion, $querycliente);
                    $row_escuelas = mysqli_fetch_assoc($cliente);
                    $_SESSION['login'] = 1;
                    $_SESSION['idEscuela'] = $row_escuelas['idEscuela'];
                    $_SESSION['nombre'] = $row_escuelas['Nombre'];
                    $_SESSION['img'] = $row_escuelas['img'];
                    $_SESSION['idTUsuario'] = $row_escuelas['idTUsuario'];
                }else if($pass == $row['password']){
                    echo json_encode('success');
                    $querycliente = "SELECT * FROM escuelas WHERE email='$email'";
                    $cliente = mysqli_query($conexion, $querycliente);
