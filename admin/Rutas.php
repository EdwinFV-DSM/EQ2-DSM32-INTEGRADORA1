...
$total_rutas_db = $sentencia->rowCount();
$paginas = $total_rutas_db / 8;
        ...
        <div class="row shadow-lg p-3 mb-5 bg-body rounded">
            ...
            <table class="table">
                ...
                <?php if ($_REQUEST['campo'] == '') { ?>
                    ...
                    $iniciar = ($_GET['pagina'] - 1) * $rutas_x_pagina;
                    // echo '<h1>'.$iniciar.'</h1>';

                    $sql_rutas = 'SELECT * FROM rutas ORDER BY "nombre" ASC LIMIT :iniciar,:narticulos';

                    $sentencia_rutas = $PDO->prepare($sql_rutas);
                    $sentencia_rutas->bindParam(':iniciar', $iniciar, PDO::PARAM_INT);
                    $sentencia_rutas->bindParam(':narticulos', $rutas_x_pagina, PDO::PARAM_INT);
                    $sentencia_rutas->execute();

                    $resultado_rutas = $sentencia_rutas->fetchAll();
                    foreach ($resultado_rutas as $rutas) : ?>

                        <tr>
                            <th scope="row"><?php echo $rutas['nombre'] ?></th>
                                                        
                            <td>
                                <a class="btn btn-success" href="<?= base_url ?>admin/operaciones/modificar_boleto.php?operacion=<?php echo $rutas['idRuta']; ?>"><i class="bi bi-pencil-square"></i></a>
                                <a class="btn btn-danger" href="<?= base_url ?>admin/boletos.php?pagina=<?php echo $_GET['pagina'] ?>&N_Operacion=<?php echo $rutas['idRuta']; ?>"><i class="bi bi-trash"></i></a>
                                <a class="btn btn-primary" href="<?= base_url ?>admin/operaciones/consulta_factura.php?operacion=<?php echo $rutas['idRuta']; ?>"><i class="bi bi-search"></i></a>
                            </td>
                        </tr>
                    <?php endforeach ?>
                    </tbody>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    <li class="page-item <?php echo $_GET['pagina'] <= 1 ? 'disabled' : '' ?>">
                        <a class="page-link" href="<?= base_url ?>admin/Rutas.php?pagina=<?php echo $_GET['pagina'] - 1 ?>">Previous</a>
                    </li>

                    <?php for ($i = 0; $i < $paginas; $i++) : ?>
                        <li class="page-item <?php echo $_GET['pagina'] == $i + 1 ? 'active' : '' ?>"><a class="page-link" href="<?= base_url ?>admin/Rutas.php?pagina=<?php echo $i + 1 ?>"><?php echo $i + 1 ?></a></li>
                    <?php endfor ?>

                    <li class=" page-item <?php echo $_GET['pagina'] >= $paginas ? 'disabled' : '' ?>">
                        <a class=" page-link" href="<?= base_url ?>admin/Rutas.php?pagina=<?php echo $_GET['pagina'] + 1 ?>">Next</a>
                    </li>
                </ul>
            </nav>
        </div>
...
/**
 * 
 * En este codigo se realiza la eliminacion de datos de la tabla obteniedo la variable N_operacion que es la id de los rutas sin
...
if ($_GET['N_Operacion']) {
    ...
    $eliminar_boleto_DB = "DELETE FROM `rutas` WHERE N_Operacion =" . $eliminar_boleto;
