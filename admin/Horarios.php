...
$sentencia = $PDO->prepare($horarios);
$sentencia->execute();
...
$total_horarios_db = $sentencia->rowCount();
$paginas = $total_horarios_db / 8;
        ...
        <div class="row shadow-lg p-3 mb-5 bg-body rounded">
            ...
            <table class="table">
                ...
                <?php if ($_REQUEST['campo'] == '') { ?>
                    ...
                    $iniciar = ($_GET['pagina'] - 1) * $horarios_x_pagina;
                    // echo '<h1>'.$iniciar.'</h1>';

                    $sql_horarios = 'SELECT * FROM horarios ORDER BY "idHorario" LIMIT :iniciar,:narticulos';

                    $sentencia_horarios = $PDO->prepare($sql_horarios);
                    $sentencia_horarios->bindParam(':iniciar', $iniciar, PDO::PARAM_INT);
                    $sentencia_horarios->bindParam(':narticulos', $horarios_x_pagina, PDO::PARAM_INT);
                    $sentencia_horarios->execute();

                    $resultado_horarios = $sentencia_horarios->fetchAll();
                    foreach ($resultado_horarios as $horarios) : ?>

                        <tr>
                            <th scope="row"><?php echo $horarios['TiempoE'] ?></th>
                                                        
                            <td>
                                <a class="btn btn-success" href="<?= base_url ?>admin/operaciones/modificar_boleto.php?operacion=<?php echo $horarios['idHorario']; ?>"><i class="bi bi-pencil-square"></i></a>
                                <a class="btn btn-danger" href="<?= base_url ?>admin/boletos.php?pagina=<?php echo $_GET['pagina'] ?>&N_Operacion=<?php echo $horarios['idHorario']; ?>"><i class="bi bi-trash"></i></a>
                                <a class="btn btn-primary" href="<?= base_url ?>admin/operaciones/consulta_factura.php?operacion=<?php echo $horarios['idHorario']; ?>"><i class="bi bi-search"></i></a>
                            </td>
                        </tr>
                    <?php endforeach ?>
                    </tbody>
...
/**
 * 
 * En este codigo se realiza la eliminacion de datos de la tabla obteniedo la variable N_operacion que es la id de los horarios sin
...
if ($_GET['N_Operacion']) {
    ...
    $eliminar_boleto_DB = "DELETE FROM `horarios` WHERE N_Operacion =" . $eliminar_boleto;
